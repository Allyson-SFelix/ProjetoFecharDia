...
class Aplicativo():
    ...
    def addNosFrames(self):
        ...
        for i in range(0,6): #dentro do frame criado - - no final salvar na lista encadeada
            ...
            entradas1.bind("<FocusOut>", lambda event,contador=i,entrada=entradas1 : self.pegarOsDados(contador, entrada))
            
            entradas2=Entry(self.frameBombas[i])
            entradas2.grid(row=1,column=1)
            # no momento em que tira o foco da caixinha ele salva o valor indo la pra funcao de pegar os dados
            entradas2.bind("<FocusOut>", lambda event,contador=i+1,entrada=entradas2 : self.pegarOsDados(contador, entrada))
    ...
    def pegarOsDados(self, i, entrada):     
        ...
        if tempValor!="":
            tempFloatValor=float(tempValor)
            if tempFloatValor%2==0:
                self.entradasTotal[i]=tempFloatValor
            else:
                self.entradasTotal[i]=tempFloatValor
        else:
            print("VAZIO")
